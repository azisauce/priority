...
export async function GET(request: NextRequest) {
  ...
  const formattedItems = await Promise.all(items.map(async (item) => {
    ...
    return {
      ...
      priceWithInterest: item.price_with_interest !== null && item.price_with_interest !== undefined ? Number(item.price_with_interest) : null,
...
export async function POST(request: NextRequest) {
  ...
  const [item] = await db("items")
    .insert({
      ...
      group_id: groupId,
      price: pricing,
      enabled_ease_option: parsed.data.enabledEaseOption ?? false,
      price_with_interest: parsed.data.priceWithInterest ?? null,
      interest_percentage: parsed.data.interestPercentage ?? 0,
      ease_period: parsed.data.easePeriod ?? 0,
